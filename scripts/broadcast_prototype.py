...
class Broadcaster:
    def __init__(self, root_dir, debug, self_pkg_name):
        ...
        self.error_pub                  = rospy.Publisher(self.error_topic, Error, 
                                                          queue_size=10)
        self.node_respawn_result_pub    = rospy.Publisher(self.node_respawn_result_topic, 
                                                          NodeRespawnResult, queue_size=10)
        self.vital_pub = None
    ...
    def raise_error(self, error_code, exception=None):
        ...
        line_no = exc_tb.lineno
        print(exc_type, fname, line_no)
        
        self.publish_response()
        self.publish_error_msg(exc_type, fname, line_no)
    ...
    def set_default_values(self):
        self.rate1                  = rospy.Rate(self.DEFAULT_VALUES["BROADCAST_THREAD_HZ"])
        self.rate1                  = rospy.Rate(self.DEFAULT_VALUES["COMMUNICATION_THREAD_HZ"])
    ...
    def set_publisher(self):
        self.broadcast_pub          = rospy.Publisher(self.broadcast_topic, Broadcast,
                                                      queue_size=10)
        self.vital_pub              = rospy.Publisher(self.vital_topic, Vital, queue_size=10)
        
    def set_subscriber(self):
        self.broadcast_sub          = rospy.Subscriber(self.broadcast_topic, Broadcast, 
                                                       self.broadcast_callback)
        self.remote_control_sub     = rospy.Subscriber(self.remote_control_topic, UInt8, 
                                                       self.remote_control_callback)    
        self.system_shutdown_sub    = rospy.Subscriber(self.system_shutdown_topic, UInt8, 
                                                       self.system_shutdown_callback)
    ...
    def publish_error_msg(self, exc_type, fname, line_no):
        msg = Error()
        msg.NodeCode        = self.node_code
        msg.ErrorCode       = self.err_code

        msg.Info.ExcType    = exc_type
        msg.Info.Fname      = fname
        msg.Info.LineNo     = line_no
